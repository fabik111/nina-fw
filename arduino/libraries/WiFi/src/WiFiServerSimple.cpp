...
WiFiServerSimple::WiFiServerSimple() :
  WiFiServerSimple(0)
{
}

WiFiServerSimple::WiFiServerSimple(uint16_t port) :
  _port(port),
  _socket(-1)
{
...
WiFiSimple WiFiServerSimple::available(uint8_t* status)
{
  ...
  for (int i = 0; i < CONFIG_LWIP_MAX_SOCKETS; i++) {
    if (_spawnedSockets[i] != -1) {
      WiFiSimple c(_spawnedSockets[i]);
  ...
  return WiFiSimple(result);
}

uint8_t WiFiServerSimple::status() {
  // Deprecated.
  return 0;
}

size_t WiFiServerSimple::write(uint8_t b)
{
  return write(&b, 1);
}

size_t WiFiServerSimple::write(const uint8_t *buffer, size_t size)
{
  size_t written = 0;
...
WiFiServerSimple::operator bool()
{
